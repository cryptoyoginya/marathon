...
3. **Файл человека.** Markdown с YAML-шапкой в приватном локальном репо `~/Documents/marathon-plans/`. Это источник правды, человек получает его вместе со страницей.
4. **Проверка и артефакт.** `scripts/render.mjs` валидирует инварианты плана (длины массивов, рост до 10%, доля длинной, фазы, подводка, дата забега), сверяет данные страницы с YAML, прогоняет смоук рантайма страницы и детерминированно сшивает HTML из `assets/plan-template.html`. Модель пишет данные, корректность и рендер проверяет код.
...
├── scripts/
│   ├── validate.mjs          инварианты plan.md
│   ├── render.mjs            сверка PLAN с YAML и сборка plan.html
│   └── test.mjs              регрессия скриптов на фикстуре
...
- Начинаем с минимума, дорабатываем по реальному опыту. Правки вносятся прямо в `SKILL.md` и референсы.
- Числа плана держатся не на дисциплине модели: инварианты и сборку страницы проверяет код, у скриптов есть тесты (`node scripts/test.mjs`).
