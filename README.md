...
4. **Проверка и артефакт.** `scripts/render.mjs` валидирует инварианты плана (длины массивов, рост до 10%, доля длинной, фазы, дата забега), сверяет данные страницы с YAML и детерминированно сшивает HTML из `assets/plan-template.html`. Модель пишет данные, корректность и рендер проверяет код.
